...
{
    [NodeMenuDisplay("Tweens/UI/TMPro/Rotate Char")]
    public class TMPRotateCharVertex : TweenVertex
    {
        [SerializeField]
        private Connection _startCnx, _endCnx, _indexCnx, _wrapperCnx, _modeCnx;
        ...
        public override Tween GenerateTween()
        {
            ...
            Vector3 start = _startCnx == null ? vw.Wrapper.GetCharScale(index) : ((ValueVertex<Vector3>)_startCnx.TargetCnx).Value;
            Vector3 end = _endCnx == null ? vw.Wrapper.GetCharScale(index) : ((ValueVertex<Vector3>)_endCnx.TargetCnx).Value;
            RotateMode mode = _modeCnx == null ? RotateMode.Fast : ((ValueVertex<RotateMode>)_modeCnx.TargetCnx).Value;

            Tween tween = vw.Wrapper
                .DORotateChar(index, end, duration, mode)
                .From(start)
                .SetDelay(delay)
                .SetAutoKill(false);
            SetEase(tween);
