...
{
    [NodeMenuDisplay("Tweens/Material/Tiling")]
    public class MaterialTilingVertex : TweenVertex
    {
        [SerializeField]
        ...
        public override Tween GenerateTween()
        {
            ...
            if (propstr == null)
            {
                if (_propIdCnx == null)
                {
                    Vector2 tiling = _value[index].mainTextureScale;
                    Vector2 start = _startCnx == null ? tiling : ((ValueVertex<Vector2>)_startCnx.TargetCnx).Value;
                    Vector2 end = _endCnx == null ? tiling : ((ValueVertex<Vector2>)_endCnx.TargetCnx).Value;
                    tween = _value[index]
                ...
                else
                {
                    ...
                    Vector2 tiling = _value[index].GetTextureScale(propid);
                    Vector2 start = _startCnx == null ? tiling : ((ValueVertex<Vector2>)_startCnx.TargetCnx).Value;
                    Vector2 end = _endCnx == null ? tiling : ((ValueVertex<Vector2>)_endCnx.TargetCnx).Value;
                    tween = _value[index]
            ...
            {
                Vector2 tiling = _value[index].GetTextureScale(propstr);
                Vector2 start = _startCnx == null ? tiling : ((ValueVertex<Vector2>)_startCnx.TargetCnx).Value;
                Vector2 end = _endCnx == null ? tiling : ((ValueVertex<Vector2>)_endCnx.TargetCnx).Value;
                tween = _value[index]
