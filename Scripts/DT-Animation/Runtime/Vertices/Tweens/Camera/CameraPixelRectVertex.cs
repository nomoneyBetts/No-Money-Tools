...
{
    [NodeMenuDisplay("Tweens/Camera/Pixel Rect")]
    public class CameraPixelRectVertex : TweenVertex
    {
        [SerializeField]
        ...
        public override Tween GenerateTween()
        {
            ...
            _value = target.pixelRect;

            GetDurationDelay(out float duration, out float delay);
            Rect start = _startCnx == null ? _value : ((ValueVertex<Rect>)_startCnx.TargetCnx).Value;
            Rect end = _endCnx == null ? _value : ((ValueVertex<Rect>)_endCnx.TargetCnx).Value;

            Tween tween = target
                .DOPixelRect(end, duration)
                .From(start)
                .SetDelay(delay)
                .SetAutoKill(false);
            SetEaseAndLoops(tween);
            SetEvents(tween);
            target.pixelRect = _value;
