...
{
    [NodeMenuDisplay("Tweens/UI/Text/Text")]
    public class TextTextVertex : TweenVertex
    {
        [SerializeField]
        ...
        public override Tween GenerateTween()
        {
            ...
            ScrambleMode mode = _modeCnx == null ? ScrambleMode.None : ((ValueVertex<ScrambleMode>)_modeCnx.TargetCnx).Value;
            string scramble = _charsCnx == null ? null : ((ValueVertex<string>)_charsCnx.TargetCnx).Value;
            bool richText = _richTextCnx != null && ((ValueVertex<bool>)_richTextCnx.TargetCnx).Value;

            Tween tween = target
                .DOText(end, duration, richText, mode, scramble)
                .From(start)
                .SetDelay(delay)
                .SetAutoKill(false);
