...
{
    [NodeMenuDisplay("Tweens/UI/TMPro/Fade Char")]
    public class TMPFadeCharVertex : TweenVertex
    {
        [SerializeField]
        ...
        public override Tween GenerateTween()
        {
            ...
            float a = vw.Wrapper.GetCharColor(index).a;
            float start = _startCnx == null ? a : ((ValueVertex<float>)_startCnx.TargetCnx).Value;
            float end = _endCnx == null ? a : ((ValueVertex<float>)_endCnx.TargetCnx).Value;

            Tween tween = vw.Wrapper
                .DOFadeChar(index, end, duration)
                .From(start)
                .SetDelay(delay)
                .SetAutoKill(false);
