...
{
    [NodeMenuDisplay("Tweens/Material/Vector")]
    public class MaterialVectorVertex : TweenVertex
    {
        [SerializeField]
        ...
        public override Tween GenerateTween()
        {
            ...
            if (propstr == null)
            {
                if (_propIdCnx == null)
                {
                    Debug.LogError("Input a property to Tween");
                    return null;
                }
                else
                {
                    int propid = ((ValueVertex<int>)_propIdCnx.TargetCnx).Value;
                    Vector4 vector = _value[index].GetVector(propid);
                    Vector4 start = _startCnx == null ? vector : ((ValueVertex<Vector4>)_startCnx.TargetCnx).Value;
                    Vector4 end = _endCnx == null ? vector : ((ValueVertex<Vector4>)_endCnx.TargetCnx).Value;
                    tween = _value[index]
                        .DOVector(end, propid, duration)
                        .From(start);
                }
            }
            else
            {
                Vector4 vector = _value[index].GetVector(propstr);
                Vector4 start = _startCnx == null ? vector : ((ValueVertex<Vector4>)_startCnx.TargetCnx).Value;
                Vector4 end = _endCnx == null ? vector : ((ValueVertex<Vector4>)_endCnx.TargetCnx).Value;
                tween = _value[index]
                    .DOVector(end, propstr, duration)
                    .From(start);
            }
