...
{
    [NodeMenuDisplay("Tweens/Material/Float")]
    public class MaterialFloatVertex : TweenVertex
    {
        [SerializeField]
        ...
        public override Tween GenerateTween()
        {
            ...
            if(propstr == null)
            {
                int propid = _propIdCnx == null ? 0 : ((ValueVertex<int>)_propIdCnx.TargetCnx).Value;
                float propVal = _value[index].GetFloat(propid);
                float start = _startCnx == null ? propVal : ((ValueVertex<float>)_startCnx.TargetCnx).Value;
                float end = _endCnx== null ? propVal : ((ValueVertex<float>)_endCnx.TargetCnx).Value;
                tween = _value[index]
                    .DOFloat(end, propid, duration)
                    .From(start);
            }
            else
            {
                float propVal = _value[index].GetFloat(propstr);
                float start = _startCnx == null ? propVal : ((ValueVertex<float>)_startCnx.TargetCnx).Value;
                float end = _endCnx== null ? propVal : ((ValueVertex<float>)_endCnx.TargetCnx).Value;
                tween = _value[index]
                    .DOFloat(end, propstr, duration)
                    .From(start);
            }
